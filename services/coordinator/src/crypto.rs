//! Poseidon2 implementation for BN254 matching Barretenberg/Noir's stdlib.
//!
//! This implements the exact same Poseidon2 permutation used by:
//! - Noir's `std::hash::poseidon2_permutation`
//! - Barretenberg's UltraHonk prover
//!
//! Parameters: t=4 (state width), d=5 (S-box), 4+56+4 rounds, BN254 scalar field.
//! Round constants sourced from Barretenberg's poseidon2_params.hpp.

use ark_bn254::Fr;
use ark_ff::{Field, PrimeField};

// ---------------------------------------------------------------------------
// Round constants from Barretenberg (poseidon2_params.hpp for BN254 t=4)
// ---------------------------------------------------------------------------

/// Internal matrix diagonal for the internal (partial) round linear layer.
const INTERNAL_MATRIX_DIAG: [&str; 4] = [
...
/// First 4 external (full) round constants, 4 values per round = 16 total.
const EXTERNAL_RC_FIRST: [[&str; 4]; 4] = [
    [
...
/// 56 internal (partial) round constants — only applied to state[0].
const INTERNAL_RC: [&str; 56] = [
...
/// Last 4 external (full) round constants, 4 values per round = 16 total.
const EXTERNAL_RC_LAST: [[&str; 4]; 4] = [
    [
...
fn hex_to_fr(h: &str) -> Fr {
    let bytes = hex::decode(h).expect("valid hex constant");
...
/// External (full round) linear layer — Barretenberg's matrix_multiplication_4x4.
#[inline]
fn external_linear_layer(state: &mut [Fr; 4]) {
...
/// Internal (partial round) linear layer.
/// M_I = diag(d0, d1, d2, d3) + ones_matrix
#[inline]
fn internal_linear_layer(state: &mut [Fr; 4], diag: &[Fr; 4]) {
    let sum: Fr = state[0] + state[1] + state[2] + state[3];
    for i in 0..4 {
...
/// Run the Poseidon2 permutation on a 4-element state.
/// Matches Barretenberg/Noir's `poseidon2_permutation` exactly.
pub fn poseidon2_permutation(state: &mut [Fr; 4]) {
    let diag: [Fr; 4] = [
        hex_to_fr(INTERNAL_MATRIX_DIAG[0]),
        hex_to_fr(INTERNAL_MATRIX_DIAG[1]),
        hex_to_fr(INTERNAL_MATRIX_DIAG[2]),
        hex_to_fr(INTERNAL_MATRIX_DIAG[3]),
    ];

    // 4 initial external (full) rounds
    for round in 0..4 {
        for j in 0..4 {
            state[j] += hex_to_fr(EXTERNAL_RC_FIRST[round][j]);
        }
        for j in 0..4 {
    ...
    for round in 0..56 {
        state[0] += hex_to_fr(INTERNAL_RC[round]);
        state[0] = sbox(state[0]);
        internal_linear_layer(state, &diag);
    }

    // 4 final external (full) rounds
    for round in 0..4 {
        for j in 0..4 {
            state[j] += hex_to_fr(EXTERNAL_RC_LAST[round][j]);
        }
        for j in 0..4 {
            state[j] = sbox(state[j]);
        }
...
// ---------------------------------------------------------------------------
// Hash functions matching Noir's commitments.nr
// ---------------------------------------------------------------------------

/// Poseidon2 hash of two field elements.
/// Equivalent to Noir: `poseidon2_permutation([a, b, 0, 0], 4)[0]`
pub fn poseidon2_hash_2(a: Fr, b: Fr) -> Fr {
    let mut state = [a, b, Fr::from(0u64), Fr::from(0u64)];
    poseidon2_permutation(&mut state);
    state[0]
}

/// Commit to a single card: H(card_value, salt).
/// Matches `commitments::commit_card` in the Noir circuit.
pub fn commit_card(card: u32, salt: &str) -> Fr {
    let card_fr = Fr::from(card as u64);
    let salt_fr = Fr::from(salt.parse::<u64>().unwrap_or(0));
    poseidon2_hash_2(card_fr, salt_fr)
}

/// Commit to a player's hand: H(card1_commit, card2_commit).
/// Matches `commitments::commit_hand` in the Noir circuit.
pub fn commit_hand(card1_commit: Fr, card2_commit: Fr) -> Fr {
    poseidon2_hash_2(card1_commit, card2_commit)
}

/// Hash two children in the Merkle tree: H(left, right).
/// Matches `merkle::hash_pair` in the Noir circuit.
pub fn hash_pair(left: Fr, right: Fr) -> Fr {
    poseidon2_hash_2(left, right)
}

/// Compute Merkle root from 64 leaves (52 card commitments + 12 zero padding).
/// Matches `merkle::compute_merkle_root` in the Noir circuit.
pub fn compute_merkle_root(leaves: &[Fr; 64]) -> Fr {
    // 64 -> 32
    let mut layer: Vec<Fr> = (0..32)
        .map(|i| hash_pair(leaves[2 * i], leaves[2 * i + 1]))
        .collect();

    // 32 -> 16
    let mut next: Vec<Fr> = (0..16)
        .map(|i| hash_pair(layer[2 * i], layer[2 * i + 1]))
        .collect();
    layer = next;
    ...
    // 4 -> 2
    let h0 = hash_pair(next[0], next[1]);
    let h1 = hash_pair(next[2], next[3]);

    // Root
    hash_pair(h0, h1)
}

/// Format a BN254 field element as a decimal string (for Noir Prover.toml).
...
mod tests {
    ...
    fn test_poseidon2_deterministic() {
        ...
        assert_eq!(h1, h2, "Poseidon2 should be deterministic");
    }

    #[test]
    fn test_poseidon2_different_inputs() {
        let h1 = poseidon2_hash_2(Fr::from(1u64), Fr::from(2u64));
        let h2 = poseidon2_hash_2(Fr::from(1u64), Fr::from(3u64));
        assert_ne!(h1, h2, "Different inputs should produce different hashes");
    }

    #[test]
    fn test_commit_card_matches_circuit() {
        // commit_card(42, 123456789) should match Noir's output
        let commitment = commit_card(42, "123456789");
        let commitment2 = commit_card(42, "123456789");
        assert_eq!(commitment, commitment2);

        // Different salt should give different commitment
        let commitment3 = commit_card(42, "987654321");
    ...
    fn test_hand_commitment() {
        let c1 = commit_card(10, "111");
        let c2 = commit_card(25, "222");
        let hand = commit_hand(c1, c2);
        let hand2 = commit_hand(c1, c2);
